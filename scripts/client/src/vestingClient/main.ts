import {
  ...
  export async function unlock(
    ...
    /**
     * Optional priority fee in microLamports.
     * e.g. 1_000_000 for 1 SOL per compute unit, or 1000 for 0.000001 SOL per compute unit, etc.
     */
    priorityMicroLamports: number = 0
  ): Promise<TransactionInstruction[]> {
    // 1) Derive the program address and vesting info
    ...
    // getContractInfo would be your function that fetches the vesting data from chain
    const vestingInfo = await getContractInfo(connection, vestingAccountKey);
  
    // 2) Build your main unlock instruction
    ...
    // 3) Put them in an array (you might want to name it something other than `instruction`)
    const instructions = [unlockIx];
  
    // 4) Optionally prepend a priority fee instruction
    if (priorityMicroLamports > 0) {
      const feeIx = ComputeBudgetProgram.setComputeUnitPrice({
        microLamports: priorityMicroLamports,
      });
      instructions.unshift(feeIx);
    }
